...
<style>
  ...
  .khung-dk-tk { min-height: calc(100vh - 160px); display:flex; align-items:center; justify-content:center; padding:24px; }
  .the-dk-tk {
    width:100%; max-width:520px; background:#fff; border-radius:16px;
    box-shadow:0 12px 36px rgba(0,0,0,.10); border:1px solid rgba(0,0,0,.06); overflow:hidden;
  }
  .dk-header {
    padding:20px 24px; background:linear-gradient(135deg,#22c55e 0%, #0ea5e9 100%);
  ...
  .dk-header .logo img { max-width:100%; max-height:100%; object-fit:contain; }
  .dk-body { padding:20px 24px 24px; }
  ...
  .ip:focus { border-color:#93c5fd; box-shadow:0 0 0 4px #dbeafe; }
  .loi-hop {
...
<div class="khung-dk-tk">
  <div class="the-dk-tk">
    ...
    <div class="dk-body">
      ...
      <form method="post" action="<?= htmlspecialchars($cfg_base_url) ?>/public/dang_ky_tai_khoan.php" autocomplete="on" novalidate>
        <div class="form-nhom">
        ...
        <div class="form-nhom" style="margin-top:4px; text-align:center">
          <span class="nhac-nho">Da co tai khoan?
            <a class="nut-phu" href="<?= htmlspecialchars($cfg_base_url) ?>/public/dang_nhap.php">Đăng nhập</a>
          </span>
        </div>
      </form>
    </div>
