...
<div class="the modal-root" data-event-id="<?= (int) $id ?>"
    style="max-width:780px;padding:0; background-color: white; color: black;">
    <div
        style="padding:12px 16px;border-bottom:1px solid rgba(0,0,0,.08);display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
    ...
    <form class="modal-form-tim" style="display:flex;gap:8px;align-items:center;padding:12px 16px">
        <input type="search" name="q" placeholder="Nhập tên (đúng hoặc chứa)" value="<?= htmlspecialchars($q) ?>"
            style="flex:1">
    ...
    <div style="max-height:60vh; overflow:auto;" class="popup-danh-sach">
        <table>
            ...
            <tbody>
                ...
                <?php if (empty($rows)): ?>
                <tr>
                    <td colspan="4"><i>Không có bản ghi phù hợp</i></td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    <div class="modal-phan-trang" data-per-page="<?= (int) $per_page ?>" data-q="<?= htmlspecialchars($q) ?>"
        style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-top:1px solid rgba(0,0,0,.08)">
        <span class="nho">Tổng: <?= $tong ?> — Trang <?= $page ?>/<?= $pages ?></span>
        <div class="nut-nhom">
            <a class="nut <?= $page <= 1 ? 'vohieu' : '' ?>" data-page="1"
                href="<?= link_popup($id, 1, $per_page, $q) ?>">⏮
                Đầu</a>
            <a class="nut <?= $page <= 1 ? 'vohieu' : '' ?>" data-page="<?= max(1, $page - 1) ?>"
                href="<?= link_popup($id, max(1, $page - 1), $per_page, $q) ?>">◀ Trước</a>
            <?php $from = max(1, $page - 2);
      $to = min($pages, $page + 2);
      for ($i = $from; $i <= $to; $i++): ?>
            <a class="nut <?= $i === $page ? 'chinh' : '' ?>" data-page="<?= $i ?>"
                href="<?= link_popup($id, $i, $per_page, $q) ?>"><?= $i ?></a>
            <?php endfor; ?>
            <a class="nut <?= $page >= $pages ? 'vohieu' : '' ?>" data-page="<?= min($pages, $page + 1) ?>"
                href="<?= link_popup($id, min($pages, $page + 1), $per_page, $q) ?>">Sau ▶</a>
            <a class="nut <?= $page >= $pages ? 'vohieu' : '' ?>" data-page="<?= $pages ?>"
                href="<?= link_popup($id, $pages, $per_page, $q) ?>">Cuối ⏭</a>
        </div>
    </div>
</div>