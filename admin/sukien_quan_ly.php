...
if ($hanh_dong === 'them' && !empty($_POST)) {
  ...
  $dv->them_su_kien(
    ...
    $_POST['gia'],
    $_POST['so_luong'],
    $_POST['thoi_gian_bat_dau'],
    $_POST['thoi_gian_ket_thuc'],
    $mo_ta_html,
    $_POST['anh_bia'] ?? null
  );
...
function dt_local($s)
{
  return $s ? date('Y-m-d\TH:i', strtotime($s)) : '';
}
...
">
        ...
        <div class="field">
            ...
            <input type="datetime-local" name="thoi_gian_bat_dau" required
                value="<?= dt_local($sua['thoi_gian_bat_dau'] ?? '') ?>">
        </div>

        <div class="field">
            <label>Thời gian kết thúc</label>
            <input type="datetime-local" name="thoi_gian_ket_thuc" required
                value="<?= dt_local($sua['thoi_gian_ket_thuc'] ?? '') ?>">
        </div>
...
<script src="https://cdn.tiny.cloud/1/l02o6fkr26gk21ln9ffowkkbb5i9mfa6zttkec9qswu7yysw/tinymce/8/tinymce.min.js"
...
(function() {
    ...
    function setStatus(msg, cls) {
        ...
        status.className = 'status-upload ' + (cls || '');
    }
    ...
    input.addEventListener('change', async function() {
        ...
        if (f.size > 5 * 1024 * 1024) {
            setStatus('File quá lớn (>5MB).', 'err');
            return;
        }
