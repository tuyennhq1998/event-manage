...
(function() {
    ...
    if (!modalWrap) {
        ...
        modalWrap.style.cssText =
            'position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.45);z-index:9999;padding:16px;';
    ...
    document.addEventListener('click', async function(e) {
        ...
        modalWrap.innerHTML =
            '<div class="the" style="padding:24px;color:#555;background:#fff">Đang tải…</div>';
        modalWrap.style.display = 'flex';
        try {
            const res = await fetch(`${BASE}/admin/bao_cao_ds_user.php?id=${encodeURIComponent(id)}`, {
                credentials: 'same-origin'
            });
            ...
            modalWrap.innerHTML =
                `<div class="the" style="padding:24px;background:#fff;color:#dc2626">Lỗi tải dữ liệu: ${err.message}</div>`;
        }
    });
...
(function() {
    ...
    async function loadModalList(id, page = 1, perPage = 10, q = '') {
        const url =
            `${BASE}/admin/bao_cao_ds_user.php?id=${encodeURIComponent(id)}&page=${page}&per_page=${perPage}&q=${encodeURIComponent(q)}`;
...
document.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.ok) {
            ...
            box.innerHTML = '<div class="the" style="background:#fee2e2;color:#991b1b">Lỗi: ' + (data.err ||
                'Không rõ') + '</div>';
        }
    } catch (err) {
        box.innerHTML = '<div class="the" style="background:#fee2e2;color:#991b1b">Lỗi: ' + err.message +
            '</div>';
    }
});
</script>
...
document.addEventListener('click', async function(e) {
    ...
    if (btnXem) {
        ...
        try {
            ...
            const res = await fetch(`${base}/admin/lien_he_xem.php?id=${encodeURIComponent(id)}`, {
                credentials: 'same-origin'
            });
            const html = await res.text();
